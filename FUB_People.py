...
def stream_data(api_key):
    ...
    pbar = tqdm(desc="Fetching first 100 people", total=100, unit=" people")
...
fetched_people_data = stream_data(api_key)
df = pd.DataFrame(fetched_people_data)
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
filename = f'first_100_people_data_{timestamp}.csv'
df.to_csv(filename, index=False)
print(f"First 100 people data fetched and saved to {filename}")

# Save the raw JSON data to a file
json_filename = f'first_100_people_data_{timestamp}.json'
with open(json_filename, 'w') as json_file:
    json.dump(stream_data(api_key), json_file, indent=4)
print(f"First 100 people raw data saved to {json_filename}")